...
AFRAME.registerComponent("space-environment", {
  schema: {
    ...
    starCount: { type: "number", default: 1400 },
    /** Fraction of stars pulled into a Milky-Way-like band rather than spread evenly. */
    bandFraction: { type: "number", default: 0.45 },
    /** Star diameter in CSS pixels. three converts to device pixels - see buildStarLayer. */
    starSize: { type: "number", default: 3.4 },
    /** Fraction of stars promoted to the brighter, larger second layer. */
    brightFraction: { type: "number", default: 0.09 },
    galaxyCount: { type: "number", default: 4 },
    moonEnabled: { type: "boolean", default: true },
    /** Degrees/second. Defaults to the shared backdrop rate; see SKY_ROTATION_DEG_PER_SEC. */
    rotationSpeed: { type: "number", default: SKY_ROTATION_DEG_PER_SEC },
    asteroidCount: { type: "number", default: 0 },
    asteroidSpeed: { type: "number", default: 0.3 },
    nebulaEnabled: { type: "boolean", default: false },
    backgroundColor: { type: "color", default: "#000006" },
  },
  ...
  init() {
    ...
    // Everything that belongs to the sky rides in here.
    this.skyGroup = new THREE.Group();
    this.skyGroup.name = "sky";
    this.el.object3D.add(this.skyGroup);
  ...
  remove() {
    if (this.skyGroup) this.el.object3D.remove(this.skyGroup);
    this.asteroids.forEach((asteroid) => this.el.object3D.remove(asteroid));
...
AFRAME.registerComponent("base-coronas", {
  ...
  addTeam(positions, color) {
    positions.forEach((position) => {
      ...
      this.el.sceneEl.object3D.add(sprite);
  ...
  tick(time) {
    ...
    this.coronas.forEach((corona) => {
      ...
      opacity *= 0.86 + 0.14 * Math.sin(time * 0.0013 + corona.phase);
  ...
  remove() {
    this.coronas.forEach(({ sprite, material }) => {
      this.el.sceneEl.object3D.remove(sprite);
      material.dispose();
    });
