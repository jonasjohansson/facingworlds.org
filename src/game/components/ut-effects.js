...
//
// The pure helpers at the top are exported for server/test/ut-effects.test.mjs; nothing in
// this file touches AFRAME at module scope, so that test can import it under a stub.
...
// ---------------------------------------------------------------------------
// pure helpers (no AFRAME, no DOM — see server/test/ut-effects.test.mjs)
// ---------------------------------------------------------------------------
...
const state = {
  ...
  lightPeak: 0,
  quadGeo: null,
  // scratch, created with the root so nothing touches AFRAME at module scope
  v: null,
};
...
function ensureRoot(sceneEl) {
  ...
  // scene with no object3D yet throws here, and a committed-but-detached group would leave
  // every pooled mesh parented to nothing.
...
function camera() {
  ...
  state.camera = (state.sceneEl && state.sceneEl.camera) || null;
  return state.camera;
}
 ...
 *
 * Not `camera.quaternion`. A-Frame's `<a-entity camera>` hangs the THREE.PerspectiveCamera
 * off the entity's own Object3D and never rotates the camera itself — look-controls writes
 * the pitch to that entity and the yaw to the rig above it — so the camera's LOCAL
 * quaternion is the identity no matter where the player is looking. Copying it onto a quad
 * aims the quad at world -Z, which for most of the map means edge-on: a 1.5 m smoke puff
 * draws as a thin vertical smear that grows and shrinks as you turn. The world quaternion
 * is the one that means "face the viewer".
 */
...
function quadGeometry() {
  ...
  state.quadGeo = new (THREEns().PlaneGeometry)(1, 1);
 ...
 * against what the contract claims; see the warning in the load callback.
 */
...
function buildPools() {
  ...
  if (ringCfg && ringModel && !state.rings) {
    ...
    for (let i = 0; i < cap(BUDGET.rings); i++) {
      ...
      if (ringModel.animations.length) {
        const clip =
          ringModel.animations.find((c) => c.name === clipName) || ringModel.animations[0];
        if (clip) {
  ...
  if (shellModel && !state.shells) {
    ...
    for (let i = 0; i < cap(BUDGET.shells); i++) {
      ...
      const { obj, mats } = cloneForPool(shellModel, AFRAME.THREE.NormalBlending);
      state.root.add(obj);
      state.shells.push({
        obj,
        mats,
  ...
  if (state.pockTex && !state.pocks) {
    ...
    for (let i = 0; i < cap(BUDGET.pocks); i++) {
      ...
      const mat = new THREE.MeshBasicMaterial({
        ...
        opacity: 1,
        depthWrite: false,
        blending: blendOf(pockCfg, "modulate"),
        side: THREE.DoubleSide,
        toneMapped: false,
        fog: false,
        polygonOffset: true,
 ...
 */
export function spawnShockBeam(sceneEl, from, to) {
 ...
 */
export function spawnShockRing(sceneEl, point, normal) {
  ...
  flashLight(
    ...
    normal,
    0x99bbff,
    GAME_CONFIG.EFFECTS.SHOCK_LIGHT_INTENSITY,
    GAME_CONFIG.EFFECTS.SHOCK_LIGHT_LIFE,
  );
  return true;
}
 ...
 */
export function spawnWallHit(sceneEl, weaponId, point, normal, dual) {
  ...
  if (!spawnPock(point, normal)) spawnDecal(sceneEl, point, normal);
...
function spawnSmokePuff(point, normal, heavy) {
  ...
  const off = by
    ? pickNum(by, [heavy ? "heavyWallHit" : "wallHit"], 0)
    : pickNum(cfg, ["offsetAlongNormalM"], 0);
  ...
  // Face the viewer on the frame it appears, not on the next tick: the system's tick may
  // already have run for this frame, and one frame of an edge-on sprite is a flicker.
 ...
 */
export function ejectShell(sceneEl, weaponId, muzzle, dir) {
 ...
 * @param {Object} result whatever hitscan() returned
 * @param {boolean} [dual] two Enforcers: half the sparks, because it fires twice as often
 */
export function drawHitscanShot(sceneEl, weaponId, muzzle, result, dual) {
  if (!sceneEl || !result) return;
  try {
    ensureRoot(sceneEl);
  } catch (e) {
    return;
  ...
  if (weaponId === "shock") {
    if (spawnShockBeam(sceneEl, muzzle, result.point)) {
      spawnShockRing(sceneEl, result.point, result.normal);
    ...
    // nothing at all.
    spawnTracer(sceneEl, muzzle, result.point);
    if (result.type !== "none") spawnImpact(sceneEl, result.point, result.normal, onPlayer);
    return;
  }
  ...
  if (onPlayer) {
    ...
    spawnImpact(sceneEl, result.point, result.normal, true);
    playAt(pickStr(pickObj(FX, ["sounds"]), ["chunkHit"], ""), result.point, 0.35, 1);
    return;
  }
  if (result.type !== "world") return;

  if (!spawnWallHit(sceneEl, weaponId, result.point, result.normal, dual)) {
    spawnImpact(sceneEl, result.point, result.normal, false);
  }
}
...
function floorUnder(pos) {
  const meshes = getWorldColliders(state.sceneEl);
...
export function disposeUtEffects() {
  ...
  // The loaded source models own the geometry and textures every clone shares.
...
async function preload(sceneEl) {
  ensureRoot(sceneEl);
  await loadContract();
  if (!FX) return;
  const THREE = THREEns();
...
AFRAME.registerSystem("ut-effects", {
  init() {
    // The pools live as long as the scene; impact-effects.js owns the single teardown path
    // both files are torn down through.
    try {
      registerEffectDisposer(disposeUtEffects);
      preload(this.sceneEl).catch((e) => console.warn("[ut-effects] preload failed:", e));
    } catch (e) {
  ...
  tick(time, dtMs) {
    ...
    updateUtEffects(Math.min(dtMs, 100) / 1000);
  },
});
