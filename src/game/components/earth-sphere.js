// Earth Sphere Component - the planet in the CTF-Face backdrop.
//
...
AFRAME.registerComponent("earth-sphere", {
  schema: {
     ...
     */
    offset: { type: "vec3", default: { x: 390, y: 120, z: -300 } },
    ...
    skyRotationSpeed: { type: "number", default: SKY_ROTATION_DEG_PER_SEC },
    atmosphereColor: { type: "color", default: "#4db2ff" },
    atmosphereIntensity: { type: "number", default: 0.8 },
    /** Warm tint of the city lights on the unlit side. */
    nightLightColor: { type: "color", default: "#ffb45a" },
    nightLightIntensity: { type: "number", default: 2.6 },
    /**
     ...
     */
    earthshine: { type: "number", default: 0.024 },
    /**
     * World-space direction *towards* the sun. Matches #key-light's position in
     * index.html (it aims at the origin), so the terminator on the planet agrees with
     * the direction the towers are lit from.
     */
    sunDirection: { type: "vec3", default: { x: 70, y: 95, z: -100 } },
  },
  ...
  init() {
    ...
    // Everything the planet owns lives under this pivot.
    this.skyPivot = new THREE.Group();
    this.skyPivot.name = "earth-sky-pivot";
    this.el.object3D.add(this.skyPivot);
  ...
  addCustomShaders(material) {
    ...
    material.onBeforeCompile = (shader) => {
      ...
      // View-space direction towards the sun, refreshed every frame in tick().
  ...
  tick(time, deltaTime) {
    ...
    const dt = Math.min(deltaTime || 16, 100) / 1000;

    if (camera) {
      // Pin to the camera: a skybox has no parallax.
      camera.getWorldPosition(this._camPos);
      this.el.object3D.worldToLocal(this._camPos);
    ...
    if (this.atmosphereMaterial && camera) {
      this._sunView.copy(this._sunWorld).transformDirection(camera.matrixWorldInverse);
  ...
  remove() {
    if (this.skyPivot) {
      this.el.object3D.remove(this.skyPivot);
    }
