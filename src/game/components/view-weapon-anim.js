...
/**
 * @param {object} root the loaded glTF scene root (el.getObject3D("mesh"))
...
export function createViewAnim(root, clips, anims, random = Math.random) {
  const THREE = window.AFRAME ? AFRAME.THREE : window.THREE;
  ...
  return {
    ...
    // A-Frame's gltf-model detaches the outgoing mesh inside that same call — so a Down
    // pose would have no frames to be seen in. See dressSlot() in first-person-weapon.js.
