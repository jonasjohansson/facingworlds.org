...
//
// THE POOLS STAY MODULE-LEVEL ON PURPOSE. Three callers share them — the local weapon
// (systems/first-person-weapon.js), ut-effects.js's fallbacks and its wall-hit decal, and
// network.js drawing every remote player's shot — and a per-instance pool would mean a
// second set of GPU buffers the moment anything held a stale reference. The class below
// is the one owner of the teardown: game.dispose() disposes systems in reverse
// registration order, and ImpactEffects.dispose() is what calls disposeImpactEffects().
...
function ensureRoot(game) {
  ...
  // Attach first: committing the group to `state` before it is parented would leave every
  // pooled mesh hanging off a detached group if the scene were not there.
  game.scene.add(root);
  state.game = game;
  state.root = root;
...
export function spawnTracer(game, from, to) {
 ...
 */
export function spawnImpact(game, point, normal, onPlayer) {
  if (!game || !game.scene) return;
  ensurePools(game);
  const E = GAME_CONFIG.EFFECTS;
 ...
 */
export function spawnDecal(game, point, normal) {
  if (!game || !game.scene) return;
  ensurePools(game);
  const E = GAME_CONFIG.EFFECTS;
  _look.copy(point).add(normal);
...
/**
 * The system: it owns when the pools are warmed and when they are released, and runs the
 * decay loop. It draws nothing itself — every spawner above is called from the weapon,
 * ut-effects.js or network.js.
 *
...
export class ImpactEffects {
  ...
  update(dt) {
    if (!state.tracers) return;
    // tick(time, dtMs) clamped a tab-throttled frame at 100 ms before dividing; the loop
    // in engine/game.js already clamps at 50 ms, and this keeps the old ceiling anyway.
    updateImpactEffects(Math.min(dt, 0.1));
  }

  dispose() {
    disposeImpactEffects();
  }
}
